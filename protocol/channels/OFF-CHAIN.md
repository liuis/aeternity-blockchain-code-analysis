...
每一方都保留一个特定于该通道的状态树。 它由所有通道数据组成：帐户，合约等，并且具有与on-chain状态树相同的结构。 离线交易更新此通道辅助树。 各方有责任在本地保留此信息。 单独结账交易提供了包含它的证据，而不是发布整个树。 每个离线更新包括在通道状态树之上应用的更新和表示它何时发生的整数值。 由于必须总是提供回合，所以提供了两个离线交易，我们可以推断出比其他交易更早。
...
##消息
 ...
 -  [控制消息]（#control-messages）
  + [`error`]（＃error）
  + [`ping` /`pong`]（#pingpong）
 -  [建立通道脱链]（＃established-channel-off-chain）
...
由于性能原因，不会在每次更新时保留。）Aeternity节点通道
fsm自动恢复状态树并验证提供的
国家实际上是最后一个相互签署的国家。
...
一旦交换了“funding_locked”消息，通道就会进入
